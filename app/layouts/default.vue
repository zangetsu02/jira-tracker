    ...
    <div class="min-h-screen bg-[var(--ui-bg)]">
        ...
        <aside
            ...
            <div class="h-[var(--header-height)] flex items-center justify-center border-b border-[var(--ui-border)]">
                <NuxtLink to="/" class="group">
                    <div
                        ...
                        <UIcon name="i-lucide-scan-search" class="w-5 h-5 text-[var(--ui-bg)]" />
                    </div>
                </NuxtLink>
            </div>
            ...
            <nav class="flex-1 py-6 flex flex-col items-center gap-2">
                <UTooltip v-for="item in navigation" :key="item.to" :text="item.label" :delay-open="0" side="right">
                    <NuxtLink :to="item.to"
                        class="relative w-11 h-11 flex items-center justify-center transition-all duration-200" :class="isActiveRoute(item.to)
            ...
            <div class="py-6 flex flex-col items-center gap-3 border-t border-[var(--ui-border)]">
                ...
                <!-- Theme Toggle -->
                <UColorModeButton size="sm" color="neutral" variant="ghost" class="w-11 h-11" />

                <!-- User Avatar -->
                <UTooltip v-if="loggedIn" :text="`Logout (${user?.email})`" :delay-open="0" side="right">
                    <button
