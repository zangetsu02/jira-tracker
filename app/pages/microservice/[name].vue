    ...
    <div class="animate-fade-in">
        ...
        <nav class="mb-6">
            <ol class="flex items-center gap-2 text-sm">
                <li>
                    <NuxtLink to="/" class="text-[var(--ui-text-muted)] hover:text-[var(--ui-text)] transition-colors">
                        Dashboard
                    </NuxtLink>
                </li>
            ...
            <div class="flex items-start justify-between gap-8">
                <div class="flex-1 min-w-0">
                    <div class="flex items-center gap-4 mb-2">
                        <div class="w-14 h-14 bg-[var(--ui-bg-muted)] flex items-center justify-center">
                            <UIcon name="i-lucide-box" class="w-7 h-7 text-[var(--ui-text-muted)]" />
                        </div>
                        <div>
                ...
                <div class="flex items-center gap-3">
                    <button v-if="!extracting"
                        ...
                        :disabled="!canExtract" :title="!microservice?.pdfFilename ? 'Carica prima un PDF' : ''"
                        @click="startExtraction">
                        <UIcon name="i-lucide-file-search" class="w-4 h-4" />
                        Estrai Use Case
                    </button>
                    <button v-if="!analyzing"
                        ...
                        :disabled="!canAnalyze" :title="!hasUseCases ? 'Estrai prima gli use case' : ''"
                        @click="startAnalysis">
                        <UIcon name="i-lucide-play" class="w-4 h-4" />
                        Analizza
                    </button>
                    <NuxtLink :to="`/analysis/${name}`"
        ...
        <!-- Loading -->
        <div v-if="pending && !microservice" class="flex items-center justify-center py-32">
            <UIcon name="i-lucide-loader-2" class="w-8 h-8 animate-spin text-[var(--ui-text-muted)]" />
        </div>

        <!-- Error -->
        <UAlert v-else-if="error" color="error" variant="soft" icon="i-lucide-alert-triangle" title="Errore"
            :description="error.message" />

        <div v-else-if="microservice" class="space-y-6">
            <!-- Error Alert -->
            <div v-if="analyzeError"
            ...
            <!-- Extraction Progress -->
            <div v-if="extracting" class="bento-item p-6">
                ...
                <div class="h-2 bg-[var(--ui-bg-muted)] overflow-hidden">
                    <div class="h-full bg-[var(--ui-info)] transition-all duration-300"
                        :style="{ width: `${extractionProgress}%` }" />
                </div>
            </div>

            <!-- Analysis Progress -->
            <div v-if="analyzing" class="bento-item p-6">
                <div class="flex items-center justify-between mb-4">
                    <div class="flex items-center gap-3">
                        <div class="w-10 h-10 bg-[var(--accent-soft)] flex items-center justify-center">
                            <UIcon name="i-lucide-loader-2" class="w-5 h-5 text-[var(--accent)] animate-spin" />
                        </div>
                        <div>
            ...
            <div class="bento-grid">
                ...
                <div class="bento-item bento-item--span-3 p-6 animate-slide-up stagger-1 report-card group">
                    <div class="report-card__header">
                        <div class="report-card__icon-wrapper"
                            ...
                            <UIcon :name="microservice.pdfFilename ? 'i-lucide-file-check' : 'i-lucide-file-x'"
                                class="w-5 h-5" />
                    ...
                    <div class="mt-5">
                        <div v-if="microservice.pdfFilename" class="report-card__file-info">
                            <p class="font-medium text-sm truncate">{{ microservice.pdfFilename }}</p>
                            <span class="report-card__badge report-card__badge--success mt-2">Caricato</span>
                        </div>
                        <div v-else class="report-card__file-info">
                            <p class="text-[var(--ui-text-muted)] text-sm">Non caricato</p>
                            <p class="text-xs text-[var(--ui-text-dimmed)] mt-1">Carica un PDF per avviare l'analisi</p>
                        </div>
                    </div>

                    <button class="report-card__action-btn mt-4" :disabled="uploadProgress" @click="triggerUpload">
                        <UIcon :name="uploadProgress ? 'i-lucide-loader-2' : 'i-lucide-upload'" class="w-4 h-4"
                ...
                <div class="bento-item bento-item--span-3 p-6 animate-slide-up stagger-2 report-card group">
                    ...
                    <div class="report-card__value-wrapper mt-5">
                        ...
                        <span v-if="hasUseCases" class="report-card__badge report-card__badge--success">
                            Pronti
                        </span>
                    </div>

                    <p v-if="!hasUseCases" class="text-xs text-[var(--ui-text-dimmed)] mt-3">
                        Estrai use case dal PDF
                    </p>

                    <button v-if="microservice?.pdfFilename && !hasUseCases"
                        class="report-card__action-btn report-card__action-btn--secondary mt-4" :disabled="!canExtract"
                        @click="startExtraction">
                        <UIcon name="i-lucide-file-search" class="w-3.5 h-3.5" />
                        Estrai
                    </button>
                </div>
                ...
                <div class="bento-item bento-item--span-3 p-6 animate-slide-up stagger-3 report-card group">
                    ...
                    <div v-if="stats.total > 0" class="mt-4 space-y-2">
                        ...
                        <div class="report-card__result-row report-card__result-row--warning">
                            <UIcon name="i-lucide-minus-circle" class="w-3.5 h-3.5" />
                    ...
                    <p v-else class="text-xs text-[var(--ui-text-dimmed)] mt-3">
                    ...
                    <div class="mt-4">
                        <div class="report-card__progress">
                            <div class="report-card__progress-track">
                                <div class="report-card__progress-fill"
                        ...
                        <span class="report-card__badge mt-3"
            ...
            <div class="grid grid-cols-1 xl:grid-cols-2 gap-6">
                ...
                <div v-if="hasUseCases" class="bento-item animate-slide-up stagger-5">
                    ...
                    <div class="max-h-[400px] overflow-y-auto">
                        <div v-for="(uc, index) in microservice?.usecases" :key="uc.id"
                            class="data-row animate-slide-in-right" :style="{ animationDelay: `${index * 0.03}s` }">
                            ...
                            <div class="flex-1 min-w-0">
                                ...
                                <p v-if="uc.description" class="text-xs text-[var(--ui-text-muted)] mt-1 line-clamp-2">
                                    {{ uc.description }}
                                </p>
                            </div>
                        </div>
                ...
                <div class="bento-item animate-slide-up stagger-6">
                    <div class="section-header">
                        ...
                        <span v-if="stats.total" class="text-xs font-mono text-[var(--ui-text-dimmed)]">
                            {{ stats.total }} totali
                        </span>
                    </div>

                    <div v-if="!microservice.analyses?.length" class="empty-state">
                        <UIcon name="i-lucide-search" class="empty-state__icon" />
                    ...
                    <div v-else class="max-h-[400px] overflow-y-auto">
                        <div v-for="(analysis, index) in microservice.analyses" :key="analysis.id"
                            ...
                            <div class="flex-1 min-w-0">
                                ...
                                <div v-if="analysis.jiraIssueKey" class="mt-1">
                                    <span class="tag tag--info font-mono text-xs">
                                        {{ analysis.jiraIssueKey }}
                                    </span>
                                </div>
                            </div>
                            <UIcon name="i-lucide-chevron-right" class="w-4 h-4 text-[var(--ui-text-dimmed)]" />
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div class="bento-item animate-slide-up stagger-7 jira-card">
                ...
                <div class="jira-card__header">
                    <div class="jira-card__title-wrapper">
                        <div class="jira-card__logo">
                            <UIcon name="i-simple-icons-jira" class="w-5 h-5" />
                        </div>
                        <div>
                            <h2 class="jira-card__title">Issue Jira</h2>
                            <p class="jira-card__subtitle">{{ jiraIssues?.total || 0 }} issue collegate</p>
                        </div>
                    </div>
                    <button class="jira-card__refresh-btn" :class="{ 'jira-card__refresh-btn--loading': jiraLoading }"
                        :disabled="jiraLoading" @click="refreshJira()">
                ...
                <!-- Loading -->
                <div v-if="jiraLoading && !jiraIssues?.issues?.length" class="jira-card__loading">
                    <div class="jira-card__loading-spinner">
                        <UIcon name="i-lucide-loader-2" class="w-6 h-6 animate-spin" />
                    </div>
                    <p class="jira-card__loading-text">Caricamento issue...</p>
                </div>

                <!-- Empty State -->
                <div v-else-if="!jiraIssues?.issues?.length" class="jira-card__empty">
                    <div class="jira-card__empty-icon">
                        <UIcon name="i-lucide-inbox" class="w-8 h-8" />
                    </div>
                    <p class="jira-card__empty-title">Nessuna issue collegata</p>
                    <p class="jira-card__empty-description">Le issue Jira verranno visualizzate qui dopo l'analisi</p>
                </div>

                <!-- Issues List -->
                <div v-else class="jira-card__list">
                    <a v-for="(issue, index) in jiraIssues.issues" :key="issue.key" :href="issue.url" target="_blank"
                        rel="noopener" class="jira-issue animate-slide-in-right"
                        :style="{ animationDelay: `${index * 0.04}s` }">
                        <div class="jira-issue__header">
                            <span class="jira-issue__key">{{ issue.key }}</span>
                            <span class="jira-issue__status" :class="{
                        ...
                        <!-- Labels -->
                        <div v-if="issue.labels?.length" class="jira-issue__tags">
                            <span 
                            ...
                            <span v-if="issue.labels.length > 2" class="jira-issue__tag jira-issue__tag--more">
            ...
            <div v-if="extractionOutput" class="bento-item animate-slide-up">
                <div class="section-header">
                    <h2 class="section-header__title">
                        Output Estrazione
                    </h2>
                    <button class="text-[var(--ui-text-muted)] hover:text-[var(--ui-text)] transition-colors"
            ...
            <div v-if="analysisOutput" class="bento-item animate-slide-up">
                <div class="section-header">
                    <h2 class="section-header__title">
                        Output Analisi
                    </h2>
